...
from car_rental_booking import GetCarRentals
...
def find_best_destinations(location):
    ...
    llm_query = f"Extract the names of the top two cities mentioned in the following snippets about {location}, and format them as a comma-separated list: {search_results}"
    ...
    return f"Provide a insigtful information of these cities: {output}"
...
# Create Tasks  
task_search = Task(
    description="Insightful short Information for the destinations in bullet points",
    agent=coach
)

task_post = Task(
    description="Create a LinkedIn post with a brief headline and a maximum of two hundred  words, focusing on upcoming AI and tech skills",
    agent=influencer
)

task_critique = Task(
    description="Refine the post for brevity, ensuring an engaging headline and keeping within a two hundred words limit",
    agent=critic
)

task_hotel = Task(
    description="Craft an engaging hotel recommendation post that captivates readers with succinct, lively bullet points and emojis. Ensure your post remains within a two hundred words limit, providing enticing details about each hotel's amenities, location highlights, and unique offerings. Infuse your recommendations with emojis to convey excitement and appeal to readers' emotions. Your goal is to entice readers to explore these hotels further and inspire them to book their next memorable stay.",
    agent=hotel
)
...
GetAirbnb(loc, checkIn, checkOut, adults)
