...
def find_best_destinations(location):
    ...
    search_query = f"Find the best destinations in {location} for a vacation, duration of {duration_string}" #change this
...
def find_best_cities(location):
    ...
    while not check:
        ...
        if all(re.match("[A-Za-z .-]+$", city.strip()) for city in cities):
            ...
            print(f"Locations are not valid, adjusting query or logging the issue.: {cities}")
...
if __name__ == "__main__":
    ...
    for location in locations:
        ...
        coach = Agent(
            ...
            backstory=f"You're an experienced travel agent with a keen eye for finding the perfect destination for any traveler. Who is traveling for {duration_string} duration", #change this
            verbose=True,
        ...
        influencer = Agent(
            role='Travel Planenr',
            goal="Craft detailed day wise travel plans with bullet points based on selected destinantions",
    ...
    # Get your crew to work!
        ...
        GetCarRental(loc, checkIn, checkOut)
